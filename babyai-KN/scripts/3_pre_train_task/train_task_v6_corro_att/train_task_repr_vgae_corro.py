...
if __name__ == '__main__':
    ...
    for epoch in tqdm(range(args.n_epoch)):
        ...
        log_lik_lst.append(log_lik_queries)
        kl_divergence_lst.append(kl_divergence_queries)
        log_lik_lst.append(log_lik_keys)
        kl_divergence_lst.append(kl_divergence_keys)
        ...
        loss_ = args.BCE_weight * BCE + args.KL_weight * KLD
        loss_all = loss_ + contrastive_loss_
        loss_all.backward()
        vgae_optimizer.step()

        writer.add_scalar('contrastive_loss', contrastive_loss_.item(), epoch)
        writer.add_scalar('BCE', BCE.item(), epoch)
        writer.add_scalar('KLD', KLD.item(), epoch)
        writer.add_scalar('loss_all', loss_all.item(), epoch)

        
        
        # queries, keys, negs = generate_seq_adj(queries, keys, negs)
