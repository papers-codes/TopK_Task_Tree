...
save_logdir = '/data2/username_high/username/BABYAI/babyai-dyth-v1.1-and-baselines/scripts/3_pre_train_task/train_task_v4_vgae/logs_models_v4_differ4_norm_h_d_128_class/'

path_root_ = save_logdir + '[06-16]11.29.18_bs_64_wo_pad_BCE_5.0_KL_0.001'
# [06-13]10.51.30_bs_64_wo_padBCE_5.0KL_0.01contra_loss_1000.0MMD_10.0
path_root_models_ = path_root_ + '/models/vae_epoch_49.pt'
path_root_npy_ = path_root_ + '/data_z_labels_mean_norm_self.npy' # 此处的vgae自带batch norm、
path_root_npy_class_n_1 = path_root_ + '/data_z_labels_clas_n_1.npy'
...
def infer_task_z_save():
    ...
    data_Y_N_1_ = torch.cat(data_Y_N_1, dim=0)
    data_all = torch.cat((data_X_, data_Y_), dim=1).detach().numpy()
    data_all_class_n_1 = torch.cat((data_Y_N_1_, data_Y_), dim=1).detach().numpy()

    np.save(path_root_npy_, data_all)
    np.save(path_root_npy_class_n_1, data_all_class_n_1)
...
def plot_clustering():
    ...
    data_tmp = data_pick(data_all, 500)
    # data_tmp = data_pooling(data_tmp)
    iris = chj_load_file(data_tmp)
    # iris = chj_load_file(data_all)
    data_pca= PCA(n_components=50).fit_transform(iris.data)
    X_tsne = TSNE(n_components=2, perplexity=200).fit_transform(data_pca)
    # X_tsne = TSNE(n_components=2, init='pca', perplexity=500).fit_transform(iris.data)
    fig = plt.figure()
