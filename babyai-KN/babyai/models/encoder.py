...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.query = nn.Linear(config.n_embd_q, config.n_embd)
        self.key = nn.Linear(config.n_embd_kv, config.n_embd)
        # self.value = nn.Linear(config.n_embd_kv, config.n_embd)
        # regularization
    ...
    def att_prob(self, q, k):
        B, T_k, C_k = k.size()   #torch.Size([batch_size, time_seq, n_embd])  batch_size一直放在最前面   128个字符 每个都映射成512向量
        B, T_q, C_q = q.size()
        # B, T_v, C_v = v.size()
        # calculate query, key, values for all heads in batch and move head forward to be the batch dim
        k = self.key(k).view(B, T_k, self.n_head, self.n_embd // self.n_head).transpose(1, 2) # (B, n_head, T_k, hs)
        q = self.query(q).view(B, T_q, self.n_head, self.n_embd // self.n_head).transpose(1, 2) # (B, n_head, T_q, hs)
        # v = self.value(v).view(B, T_v, self.n_head, self.n_embd // self.n_head).transpose(1, 2) # (B, n_head, T_v, hs)
        ...
        # y = y.transpose(1, 2).contiguous().view(B, T_q, self.n_embd) # re-assemble all head outputs side by side
                                                         # 调用view之前最好先contiguous,x.contiguous().view()  view需要tensor的内存是整块的
        # output projection
        # y = self.resid_drop(self.proj(y))

        # output att_probs  ### mean pooling 平均每个头的概率；；；
        att_probs_ = att_probs.mean(1)  # # torch.Size([bs, n_head, t_q, t_kv]) --> # torch.Size([bs, t_q, t_kv])
        att_probs_ = att_probs_.transpose(1, 2)  # torch.Size([bs, t_q, t_kv]) --> # torch.Size([bs, t_kv, t_q])
        return att_probs_
...
class QueryMLPEncoder(nn.Module):
    def __init__(self,
                 # network size
                 ...
                 state_size=2,
                 normalize_z=False,
                 task_embedding_size=5
                 ):
        super(QueryMLPEncoder, self).__init__()
        ...
        self.encoder = FlattenMlp(input_size=self.input_size,
        ...
        self.pred = MLPDecoder(input_size=task_embedding_size*2,
                hidden_size_lst=[64, 128],
                normalize_z=False,
                obs_size=self.state_size
                ##### \delete{因为 acmodel的最后一层前传的时候的隐空间的size是128}
        )  ### 重构loss去训练
    ...
    def forward(self, obs, action, rew):
        ...
        out = self.encoder(obs, action, rew)
...
class MLPEncoder(nn.Module):
    def __init__(self,
        ...
        if self.recon_flag:
            self.input_size = self.state_size + self.action_size + self.reward_size + self.term_size
        else:
            self.input_size = self.state_size*2 + self.action_size + self.reward_size + self.term_size
        # self.input_size = state_size*2+action_size+reward_size+term_size
        ...
        #                             hidden_sizes=[task_embedding_size for i in range(1)],
        #                             layer_norm=self.normalize)
    ...
    def forward(self, obs, action, reward, next_obs=None, term=None):
        ...
        if self.recon_flag:
            out = self.encoder(obs, action, reward, term) if self.use_termination \
                else self.encoder(obs, action, reward)
            out = self.encoder_2(out)
            if not self.normalize_z:
                return out
            else:
...
class MLPDecoder(nn.Module):
    def __init__(self,
        ...
        self.hidden_size_lst = hidden_size_lst
        self.normalize_z = normalize_z
        self.obs_size=obs_size
        self.decoder = FlattenMlp(input_size=self.input_size,
                                    output_size=hidden_size_lst[-1],
                                    hidden_sizes=hidden_size_lst[:-1],
                                    # layer_norm=self.normalize
                                    )
        self.decoder_2 = FlattenMlp(input_size=hidden_size_lst[-1],
                                    output_size=self.obs_size,
                                    hidden_sizes=[self.obs_size for i in range(1)],
                                    # layer_norm=self.normalize
                                    )
    def forward(self, z):
        out = self.decoder(z)
