...
class ACModel(nn.Module, babyai.rl.RecurrentACModel):
    def __init__(self, obs_space, action_space,
        ...
        self.image_conv = nn.Sequential(*[
            *([ImageBOWEmbedding(obs_space['image']*2, 128)] if use_bow else []), ### index out of range: can try :obs_space['image']*2
            *([nn.Conv2d(
        ...
        self.actor = nn.Sequential(
            nn.Linear(self.embedding_size + 5 , 64),  ### 5 是  task_embddings size
            nn.Tanh(),
    ...
    def forward(self, obs, memory, task_tree=None, instr_embedding=None):
        ...
        if not task_tree:
            ...
            # visited = torch.min(task_tree.tree_visited, 1)[1] ### index out of range: can try 
            # for i in range(task_tree.num_procs):
            # #     task_tree.tree_visited[i][visited[i]] = 1
            # task_num = torch.gather(task_tree.tree_node, 1, visited[:,None]) # 2,1
            # # import pdb
            # # pdb.set_trace()
            # extra_predictions['task_num'] = task_num.tolist()
            
            
            flatten_obs = torch.transpose(torch.transpose(obs.image, 1, 3), 2, 3).reshape(task_tree.num_procs, -1)
            all_task_embs = task_tree.pre_subtask_encoder(flatten_obs, task_tree.last_act, task_tree.last_r) ### 得到任务的嵌入  torch.Size([2, 4, 5])
            query_embs = task_tree.query_encoder(flatten_obs, task_tree.last_act, task_tree.last_r)
            
            cur_visited = []  ### current visit position
            for i in range(task_tree.num_procs):
                cur_vis = torch.nonzero(task_tree.tree_visited[i])[-1][0].int().item()
                cur_visited.append(cur_vis)
                #### judge whether to end the visited
                query_em = query_embs[i]
                cur_task_num = task_tree.tree_node[i][cur_vis].int().item()
                cur_task_em = all_task_embs[i][cur_task_num]
                simi = cosine_similarity(query_em.cpu().numpy()[None,], cur_task_em.cpu().numpy()[None,]) # 余弦相似度的取值范围是[-1,1]，相同两个向量的之间的相似度为1。
                if simi < 0.5: ## change task
                    if cur_vis < task_tree.N-1:
                        task_tree.tree_visited[i][cur_vis+1] = 1
                        task_tree.args.subtask_mean_rew[cur_task_num] = (task_tree.args.subtask_mean_rew[cur_task_num] + task_tree.subtask_duration_rewards[i]) / (task_tree.args.subtask_count[cur_task_num] + 1)
                    ##### compute the culmulative rews
                else:
                    ## culmulative reward
                    task_tree.subtask_duration_rewards[i] += 0.9*task_tree.last_r[i][0]
                    
                 
            cur_visited_ = torch.tensor(cur_visited).cuda()[:,None]
            task_num = torch.gather(task_tree.tree_node, 1, cur_visited_) # 2,1
            
            task_tree.achor_node = task_num[:,0]
            task_tree.achor_edge = torch.gather(task_tree.tree_edges, 1, cur_visited_)[:,0]
            
            extra_predictions['task_num'] = task_num.tolist()
            # task_tree.achor_node = 
    ...
    # def encode_obs(self, obs, memory, instr_embedding=None):
