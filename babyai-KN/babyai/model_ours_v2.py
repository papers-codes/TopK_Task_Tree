...
class ACModel(nn.Module, babyai.rl.RecurrentACModel):
    def __init__(self, obs_space, action_space,
        ...
        # data_4_task_new_Open_Goto20000_UnblockPickup_PutNextLocal
        task_file_path = ['/home/username/data/BABYAI/data_new/best_model/BabyAI-Open-v0_ppo_bow_endpool_res_gru_mem_seed1_22-06-13-11-15-58_best', \
            '/home/username/data/BABYAI/data_new/best_model/BabyAI-GoTo-v0_ppo_bow_endpool_res_gru_mem_seed1_22-06-13-11-16-06_best/',\
            '/home/username/data/BABYAI/data_new/best_model/BabyAI-UnblockPickup-v0_ppo_bow_endpool_res_gru_mem_seed1_22-07-10-07-31-44_best',\
            '/home/username/data/BABYAI/data_new/best_model/BabyAI-PutNextLocal-v0_ppo_bow_endpool_res_gru_mem_seed1_22-07-10-07-19-06_best']
        layers = []
        for i in task_file_path:
           layers += [utils.load_model(i, raise_not_found=True)]
        self.load_ac_models = nn.ModuleList(layers)
        ...
        self.image_conv = nn.Sequential(*[
            *([ImageBOWEmbedding(obs_space['image']*2, 128)] if use_bow else []),
            *([nn.Conv2d(
        ...
        self.actor = nn.Sequential(
            nn.Linear(self.embedding_size + 5 , 64),  ### 5 :  task_embddings size
            nn.Tanh(),
        ...
        self.critic = nn.Sequential(
            nn.Linear(self.embedding_size + 5 , 64),
            nn.Tanh(),
            nn.Linear(64, 1)
        )
    ...
    def pred_actions(self, pred_obs):
        x = self.actor(pred_obs) # 1.128 --> 1.7
        dist = Categorical(logits=F.log_softmax(x, dim=1))
        return dist

    def forward_v2(self, obs, memory, task_tree=None, instr_embedding=None):
        # task_tree.tree_node, task_tree.tree_edges, task_tree.tree_visited
        task_all_forw = [self.load_ac_models[i](obs, memory) for i in range(4)]
        dist_p, value, memory_, extra_predictions = [], [], [], []
        for i in range(task_tree.tree_node.shape[0]):
            res = task_all_forw[task_tree.tree_node[i,0].int()]
            dist_p.append(res['dist'].logits[[i]])
            value.append(res['value'][[i]])
            memory_.append(res['memory'][[i]])
            # extra_predictions.append(res['extra_predictions'][[i]]) 
        
        return {'dist': Categorical(logits=torch.vstack(dist_p)), 'value': torch.hstack(value), \
        'memory': torch.vstack(memory_), 'extra_predictions': {}, 'task_embs' : torch.zeros(len(dist_p), 5).cuda()}


    
    def forward_train(self, obs, memory, task_emb, instr_embedding=None):
        # task_tree.tree_node, task_tree.tree_edges, task_tree.tree_visited
        
        if self.use_instr and instr_embedding is None:
            instr_embedding = self._get_instr_embedding(obs.instr) # 1.6 -- > 1.128 use instruction
        ...
        x = torch.transpose(torch.transpose(obs.image, 1, 3), 2, 3) # 1.3.7.7

        if 'pixel' in self.arch:
            x /= 256.0
        x = self.image_conv(x)  #  x: torch.Size([64, 128, 7, 7])
        if self.use_instr:
            for controller in self.controllers:
                out = controller(x, instr_embedding) # 1.128.7.7, 1.128 --> 1.128.7.7
                if self.res:
                    out += x
                x = out
        x = F.relu(self.film_pool(x)) # 1.128.7.7 --> torch.Size([1, 128, 1, 1])
        x = x.reshape(x.shape[0], -1) # 1.128

        if self.use_memory:
            hidden = (memory[:, :self.semi_memory_size], memory[:, self.semi_memory_size:])  # 1.128 # 1.128
            hidden = self.memory_rnn(x, hidden) # 1.128# 1.128
            embedding = hidden[0] # 1.128
            memory = torch.cat(hidden, dim=1) # 1.256
        else:
            embedding = x
        ...
        embedding = torch.cat((embedding, task_emb), dim=-1)

        x = self.actor(embedding) # 1.128 --> 1.7
        dist = Categorical(logits=F.log_softmax(x, dim=1))
        x = self.critic(embedding)
        value = x.squeeze(1)
        

        return {'dist': dist, 'value': value, 'memory': memory, 'extra_predictions': extra_predictions, 'task_embs' : task_emb}



    def forward(self, obs, memory, task_tree=None, instr_embedding=None):
        # task_tree.tree_node, task_tree.tree_edges, task_tree.tree_visited
        if self.use_instr and instr_embedding is None:
            instr_embedding = self._get_instr_embedding(obs.instr) # 1.6 -- > 1.128 use instruction
        if self.use_instr and self.lang_model == "attgru":
        ...
        if not task_tree:
            if len(embedding.shape) != 2:
                import pdb
                pdb.set_trace()
            task_embs = torch.zeros(embedding.shape[0], 5).cuda()
        else:
            # print(task_tree.tree_node) # 2.3
            visited = torch.min(task_tree.tree_visited, 1)[1] ### newest visit position
            for i in range(task_tree.num_procs): # to-do: judge whether to end the visit position
                task_tree.tree_visited[i][visited[i]] = 1
            task_num = torch.gather(task_tree.tree_node, 1, visited[:,None]) # 2,1

            flatten_obs = torch.transpose(torch.transpose(obs.image, 1, 3), 2, 3).reshape(task_tree.num_procs, -1)
            all_task_embs = task_tree.pre_subtask_encoder(flatten_obs, task_tree.last_act, task_tree.last_r) ### get task embedding  torch.Size([2, 4, 5])

            task_num_ = task_num.expand(task_tree.num_procs,all_task_embs.shape[-1])[:,None].cuda().long()  # 2.1 --> 2.1.5
            task_embs = torch.gather(all_task_embs, 1, task_num_)[:,0]  # 2.5

        
        embedding = torch.cat((embedding, task_embs), dim=-1)
        x = self.actor(embedding) # 1.128 --> 1.7
        dist = Categorical(logits=F.log_softmax(x, dim=1))
        x = self.critic(embedding)
        value = x.squeeze(1)

        return {'dist': dist, 'value': value, 'memory': memory, 'extra_predictions': extra_predictions, 'task_embs' : task_embs}
