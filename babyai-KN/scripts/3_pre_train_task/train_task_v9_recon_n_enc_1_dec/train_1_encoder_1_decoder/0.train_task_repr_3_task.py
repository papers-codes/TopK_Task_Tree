...
from babyai.models.m_enc_1_dec import one_encoder_1_decoder
...
args.save_logdir = './logs_models_v9_3_tasks_new/'
...
args.loader_path = './data_4_task_new_Open_Goto20000_UnblockPickup_PutNextLocal.npy'
...
def vis_sample_embeddings(random_flag, npy_loader, save_path):
    ...
    encodings = torch.stack([m_encoder_1_decoder_.encoders[0](obs_context[:,i], actions_context[:,i], rewards_context[:,i], next_obs_context[:,i]) for i in tasks])
...
if __name__ == '__main__':
    ...
    #         ).to(ptu.device)
    # print(decoder)
    # encoder_optimizer = torch.optim.Adam(list(encoder.parameters())+list(decoder.parameters()), lr=args.encoder_lr)

    m_encoder_1_decoder_ = one_encoder_1_decoder(args)
    encoder_optimizer = torch.optim.Adam(m_encoder_1_decoder_.parameters(), lr=args.encoder_lr)
    ...
    last_task = npy_loader[[-1]]  # (1, 400000, 303)
    last_task[:,:,-1] = last_task[:,:,-1]-1 ### Change the id of the last task from 4 to 3

    npy_loader = np.concatenate((npy_loader[:2], last_task), axis=0)
    print('npy_loader::::::::::::::::::', npy_loader.shape)

    for epoch in tqdm(range(args.n_epoch)): # self.training_mode(True)
        m_encoder_1_decoder_.train(True)
        ...
        label_neg_unique = torch.unique(label_neg)
        b_dot_N = args.contrastive_batch_size * args.n_negative_per_positive # 64 * 16
        q_z = m_encoder_1_decoder_.encoders[0].forward(obs_q, actions_q, rewards_q, next_obs_q) # 64,5
        k_z = m_encoder_1_decoder_.encoders[0].forward(obs_k, actions_k, rewards_k, next_obs_k) # 64.5 
        
        #########  method 1 begins #### qicker than method 2 about 0.1s every 100 times
        # start_time = time.time()  #
        neg_z_ = torch.stack([m_encoder_1_decoder_.encoders[0].forward(obs_neg.reshape(b_dot_N, -1), actions_neg.reshape(b_dot_N, -1), \
            rewards_neg.reshape(b_dot_N, -1), next_obs_neg.reshape(b_dot_N, -1)).view(args.contrastive_batch_size, \
                args.n_negative_per_positive, -1) for i in label_neg_unique])  # torch.Size([3, 64, 16, 5])
        # neg_z = 
        label_neg_copy = label_neg.clone() ## 以下是将 task index 替换成下标
        for i in range(args.num_tasks-1):
            index_ = i * torch.ones_like(label_neg)  # 64.16.1
            label_neg_copy = torch.where(label_neg_copy==label_neg_unique[i],index_,label_neg_copy)
            # label_neg_unique:
        label_neg_copy = torch.tensor(label_neg_copy, dtype=torch.int64)                     # torch.Size([64, 16, 1])
        label_neg_copy_ = torch.repeat_interleave(label_neg_copy, neg_z_.shape[-1], dim=-1)  # torch.Size([64, 16, 5])
        neg_z = neg_z_.gather(0, label_neg_copy_[None])[0]  # 64.16.5
        # end_time = time.time()  #
        # print('11111 %f second' % (end_time - start_time))
        #########  method 1 ends ####

        #########  method 2 begins #### slower than method 1 about 0.1s  every 100 times
        # start_time = time.time()  #
        # neg_index = torch.tensor([label_neg[i][0][0] for i in range(args.contrastive_batch_size)], dtype=torch.int64)
        # neg_z_2 = torch.stack([m_encoder_1_decoder_.encoders[neg_index[i]-1].forward(obs_neg[i], actions_neg[i], \
        #     rewards_neg[i], next_obs_neg[i]) for i in range(args.contrastive_batch_size)])
        # end_time = time.time()  #
        # # print('22222 %f second' % (end_time - start_time))
        #########  method 2 ends ####
        ...
        #### reconstructive loss
        pred_obss_q = m_encoder_1_decoder_.decoder(q_z)
        pred_obss_k = m_encoder_1_decoder_.decoder(k_z)
        pred_obss_neg = m_encoder_1_decoder_.decoder(neg_z)
        obs_loss = (next_obs_q - pred_obss_q).pow(2).mean() + (next_obs_k - pred_obss_k).pow(2).mean() + (next_obs_neg - pred_obss_neg).pow(2).mean()

        encoder_optimizer.zero_grad()
        (contrastive_loss_+obs_loss).backward()
        ...
        if epoch % 20000 == 0:
            ...
            torch.save(m_encoder_1_decoder_, path)
