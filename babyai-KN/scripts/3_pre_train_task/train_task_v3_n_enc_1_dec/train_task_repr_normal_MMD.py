...
parser.add_argument("--infonce_weight", type=float, default=1)
parser.add_argument("--MMD", type=bool, default=True)
parser.add_argument("--disentanglement_penalty", type=float, default=1)



args = parser.parse_args()
args.save_logdir = './logs_models_v3_MMD'
args.n_epoch = 100
...
args.log_time = cur_time.strftime("[%m-%d]%H.%M.%S") + 'KL_mul_' + str(args.KL_weight) + args.contra_loss + '_MMD_' + str(args.MMD)
            ...
            if args.MMD:
                ...
                for j in range(5):  ## 5  tasks
                    ...
                    if len(index_positive_keys) > 0:
                        positive_keys = torch.index_select(z, 0, index_positive_keys)
                        negative_keys = torch.index_select(z, 0, index_negative_keys)
                        loss_ = mmd_(positive_keys, negative_keys, gammas=gammas) 
                        loss_mmd_lst.append(loss_)
                loss_mmd_ = torch.stack(loss_mmd_lst).mean() * args.disentanglement_penalty
                writer.add_scalar('MMD', loss_mmd_.item(), (epoch+1)*i*args.batch_size_)
                loss_all += loss_mmd_
