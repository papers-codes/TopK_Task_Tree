...
# Returns the performance of the agent on the environment for a particular number of episodes.
def batch_evaluate(agent, encoder_tuple, env_name, args, seed, episodes, return_obss_actions=False, pixel=False):
    ...
    n_actions = 7
    num_procs = num_envs
    task_tree = TaskTree(num_procs, N, K, args)
    ...
    for i in range((episodes + num_envs - 1) // num_envs):
        ...
        construct_num = np.zeros((num_envs,), dtype='int64')
        No_construct_num = np.zeros((num_envs,), dtype='int64')
        ...
        while (num_frames == 0).any():
            ...
            #     reconstrction_flag = True
            if task_tree.tree_visited[:,-1].any(): ###### 64,3  ;; array.any()判断其中一个为true都为true
                reconstrction_flag = True
            ...
            output_dict = agent.act_batch(many_obs, task_tree)
            action = output_dict['action']
            # import pdb
            # pdb.set_trace()
            task_num = output_dict['task_num']
            ...
            ######## mask task ::: some proc is done
            already_done_flag = already_done[:, None] * -1
            task_num_true = np.where(already_done[:, None]>0, already_done_flag, np.array(task_num))
            logs['task_num'].extend([task_num_true.tolist()])
        ...
        logs["construct_num"].extend(list(construct_num))
        logs["No_construct_num"].extend(list(No_construct_num))
        if return_obss_actions:
