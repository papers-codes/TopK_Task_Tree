...
from babyai.models.ae.autoencoder import AutoEncoder
from babyai.models.ae.ae_models import VAETaskEncoder, ProbabilisticTaskDecoder, TaskEncoder, TaskDecoder
...
class IndependentMATE(AutoEncoder):
    ...
    def encode(self, obss, acts, rews, hiddens, no_grads=False):
        ...
        :param obss: observation of each agent (num_agents, parallel_envs, obs_space)   2,10,166
        (torch.Size([10, 166]), torch.Size([10, 166]))
        ...
        for obs, act, rew, hidden, encoder, act_dim in zip(obss, acts, rews, hiddens, self.encoders, self.action_dims):
            ...
            task_embs.append(task_emb) # [torch.Size([10, 20]), torch.Size([10, 20])]
            hidden_list.append(hidden) # [torch.Size([10, 64]), torch.Size([10, 64])]
    ...
    def update(self, obss, acts, hiddens, rews, next_obss, done_mask):
        ...
        ] # [torch.Size([5, 10, 4]), torch.Size([5, 10, 4])]

        # mask out entries where episode has terminates (do not make predictions across episodes)
        mask = done_mask == 1.0
        joint_obs = torch.concat(obss, dim=-1)[mask] # torch.Size([5, 10, 332])[mask]  --> torch.Size([50, 332])
        joint_act_onehots = torch.concat(act_onehots, dim=-1)[mask] # torch.Size([5, 10, 8])[mask]  --> torch.Size([50, 8])
        joint_rews = torch.concat(rews, dim=-1)[mask] # torch.Size([5, 10, 2])[mask] --> torch.Size([50, 2])
        ...
        joint_obs = joint_obs.expand(self.n_agents, *joint_obs.shape) # torch.Size([50, 332]) --> torch.Size([2, 50, 332])
        ...
        for i, (obs, act_onehot, rew, hidden, encoder) in enumerate(zip(obss, act_onehots, rews, hiddens, self.encoders)):
            # mask out entries where episode has terminates (do not make predictions across episodes)
            # 
            obs = obs[mask] # torch.Size([5, 10, 166])  torch.Size([50, 166])
            ...
            if self.encoder_type == 'vae':
                _, mu, log_var, z, _ = encoder(obs, act_onehot, rew, hidden) # torch.Size([50, 10]) torch.Size([50, 10]) torch.Size([50, 10])
                task_emb = z
                kl_loss += -0.5 * torch.sum(1 + log_var - mu.pow(2) - log_var.exp())
            else:
                task_emb, _ = encoder(obs, act_onehot, rew, hidden)
            task_embs.append(task_emb)
        task_embs = torch.stack(task_embs, dim=0) # torch.Size([2, 50, 10])
