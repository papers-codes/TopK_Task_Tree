...
class ModelAgent(Agent):
    ...
    def __init__(self, model_or_name, obss_preprocessor, argmax):
        ...
        # self.query_encoder, self.pre_subtask_encoder, self.att = encoder_tuple

    # def act_batch(self, many_obs, z_tensor=None):
    def act_batch(self, many_obs, task_tree):
        # self.num_procs = len(many_obs)
        # self.N = 3
        ...
        ######################## vgae 
        # vgae_inputs = self.vage_processing_inputs((preprocessed_obs, self.action_, self.reward_, self.preprocessed_obs_next_)) # torch.Size([64, 302])

        with torch.no_grad():
            # # model_results = self.model(preprocessed_obs, z_tensor, self.memory) # torch.Size([256, 7, 7, 3])
                 
            model_results = self.model(preprocessed_obs, self.memory, task_tree) # torch.Size([256, 7, 7, 3])
            dist = model_results['dist']
            value = model_results['value']
            self.memory = model_results['memory']
            # import pdb
            # pdb.set_trace()
            task_num = model_results['extra_predictions']['task_num']
