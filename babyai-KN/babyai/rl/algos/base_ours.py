...
class BaseAlgo_Ours(ABC):
    """The base class for RL algorithms."""

    def __init__(self, envs, acmodel, encoder_tuple, args, num_frames_per_proc, discount, lr, gae_lambda, entropy_coef,
        ...
        self.n_actions = 7
        self.N, self.K = args.N, args.K
        self.task_tree = TaskTree(self.num_procs, self.N, self.K, args)
    ...
    def collect_experiences(self):
        ...
        for i in range(self.num_frames_per_proc): # 40
            ...
            if self.task_tree.tree_visited[:,-1].any(): ###### 64,3  ;; array.any()判断其中一个为true都为true
                reconstrction_flag = True
            
            self.task_tree.set_tree(self.query_encoder, self.pre_subtask_encoder, self.att)
            ...
            with torch.no_grad():
                model_results = self.acmodel(preprocessed_obs, self.memory * self.mask.unsqueeze(1), self.task_tree) # memeory 64*256  mask 64*1  --> {'dist':Categorical [64,7]; 'value' 64,7 'memory': 64,256 'ex..'}
                # model_results = self.acmodel(preprocessed_obs, self.memory * self.mask.unsqueeze(1)) # memeory 64*256  mask 64*1  --> {'dist':Categorical [64,7]; 'value' 64,7 'memory': 64,256 'ex..'}
            ...
            if i %20 == 0 :
                print('*'*20, self.args.subtask_mean_rew, self.args.subtask_count)
            ...
            self.nodes[i] = self.task_tree.achor_node.cuda()
            self.edges[i] = self.task_tree.achor_edge.cuda()
